...
export interface HydroInputs {
  ...
  // NASA POWER enrichment (optional — upgrades PET from Blaney-Criddle → FAO-56 Penman-Monteith)
  /** Annual mean daily solar radiation, kWh/m²/day (from NASA POWER) */
  solarRadKwhM2Day?: number;
  /** Annual mean 10-m wind speed, m/s (from NASA POWER) */
  windMs?: number;
  /** Annual mean 2-m relative humidity, % (from NASA POWER) */
  rhPct?: number;
  /** Site latitude in decimal degrees (positive N) — required to enable Penman-Monteith */
  latitudeDeg?: number;
...
export interface HydroMetrics {
  ...
  // PET provenance — which model produced petMm ('penman-monteith' when NASA POWER present)
  petMethod: PetMethod;
}
...
export function computeHydrologyMetrics(inputs: HydroInputs): HydroMetrics {
  ...
  // ── Evapotranspiration (FAO-56 Penman-Monteith when NASA POWER present,
  //     else Blaney-Criddle fallback) ──────────────────────────────────────
  // Raw PET is unconstrained by precipitation (true atmospheric demand).
  const petResult = computePet({
    annualTempC,
  ...
  const petMm = petResult.petMm;
  const petMethod = petResult.method;
  const rawPetMm = petMm;
