...
export const MARKET_GARDEN_PRIMARY_OBJECTIVES: readonly PlanStratumObjective[] = [
  ...
  obj({
    ...
    checklist: [
      ...
      ck('mgd-s1-production-targets-sales-c2', 'Set production volume targets per category'),
      ck('mgd-s1-production-targets-sales-c3', 'Define target growing area required to meet volume targets'),
      ck('mgd-s1-production-targets-sales-c4', 'Define sales model - direct, farmers market, restaurant, CSA, wholesale, or combination'),
  ...
  obj({
    ...
    checklist: [
      ...
      ck('mgd-s1-growing-system-philosophy-c3', 'Define external input policy - what inputs are acceptable and what are prohibited'),
      ck('mgd-s1-growing-system-philosophy-c4', 'Define soil health priority - how soil biology guides all production decisions'),
      ck('mgd-s1-growing-system-philosophy-c5', 'Document philosophy as design constraint - all Tier 3-4 decisions evaluated against it'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s1-market-channels-c1', 'Define primary sales channels and their share of total revenue'),
      ck('mgd-s1-market-channels-c2', 'Define customer relationships - CSA members, restaurant accounts, market regulars'),
      ck('mgd-s1-market-channels-c3', 'Define harvest day and delivery schedule per channel'),
      ck('mgd-s1-market-channels-c4', 'Define pack standards, labelling, and presentation requirements per channel'),
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s2-soil-fertility-bed-potential-c1', 'Conduct soil tests across proposed growing areas - pH, organic matter, nutrients, trace minerals'),
      ck('mgd-s2-soil-fertility-bed-potential-c2', 'Map soil variation across the site - identify premium production zones'),
      ck('mgd-s2-soil-fertility-bed-potential-c3', 'Assess drainage class in proposed bed areas'),
      ck('mgd-s2-soil-fertility-bed-potential-c4', 'Identify contamination risk - chemical residues, heavy metals from previous land use'),
      ck('mgd-s2-soil-fertility-bed-potential-c5', 'Assess compaction in proposed growing areas'),
      ck('mgd-s2-soil-fertility-bed-potential-c6', 'Define fertility gap - what must be built before target production is achievable'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s2-water-access-irrigation-c1', 'Identify all available water sources - mains, bore, dam, rainwater'),
      ck('mgd-s2-water-access-irrigation-c2', 'Assess yield and reliability of each source through driest production months'),
      ck('mgd-s2-water-access-irrigation-c3', 'Measure or estimate pressure at proposed irrigation points'),
      ck('mgd-s2-water-access-irrigation-c4', 'Assess distance from source to growing areas - relevant to pipe sizing'),
      ck('mgd-s2-water-access-irrigation-c5', 'Define maximum irrigated area supportable from available source'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s2-landscape-vectors-c1', 'Map surrounding land uses within 1km'),
      ck('mgd-s2-landscape-vectors-c2', 'Identify neighbouring spray programmes - herbicide, fungicide, insecticide risk'),
      ck('mgd-s2-landscape-vectors-c3', 'Assess prevailing wind direction relative to spray risk sources'),
      ck('mgd-s2-landscape-vectors-c4', 'Assess drinking and irrigation water catchment contamination risk'),
      ck('mgd-s2-landscape-vectors-c5', 'Identify any GMO crop risks relevant to seed saving or organic status'),
      ck('mgd-s2-landscape-vectors-c6', 'Note any industrial or chemical contamination sources'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s3-irrigation-water-quality-c1', 'Test irrigation water for biological contamination - E. coli, pathogens - relevant for food safety'),
      ck('mgd-s3-irrigation-water-quality-c2', 'Test for chemical contamination relevant to landscape vector findings'),
      ck('mgd-s3-irrigation-water-quality-c3', 'Test pH and EC - relevant to crop uptake and fertigation'),
      ck('mgd-s3-irrigation-water-quality-c4', 'Measure static and dynamic pressure at source and at furthest irrigation point'),
      ck('mgd-s3-irrigation-water-quality-c5', 'Confirm water quality meets food safety and certification requirements'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s3-pest-disease-weed-pressure-c1', 'Map weed species by zone - density, spread, and seed bank assessment'),
      ck('mgd-s3-pest-disease-weed-pressure-c2', 'Identify key pest species and their population indicators'),
      ck('mgd-s3-pest-disease-weed-pressure-c3', 'Record known disease pressure for intended crop categories'),
      ck('mgd-s3-pest-disease-weed-pressure-c4', 'Assess soil-borne disease risk from previous cropping history'),
      ck('mgd-s3-pest-disease-weed-pressure-c5', 'Identify beneficial predator populations that reduce pest pressure'),
      ck('mgd-s3-pest-disease-weed-pressure-c6', 'Prioritise pressures by production risk'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s4-crop-rotation-bed-layout-c1', 'Define rotation system - block rotation, family rotation, or bed-by-bed'),
      ck('mgd-s4-crop-rotation-bed-layout-c2', 'Define rotation interval per crop family - minimum gap between same-family crops'),
      ck('mgd-s4-crop-rotation-bed-layout-c3', 'Calculate bed count required for rotation at defined production targets'),
      ck('mgd-s4-crop-rotation-bed-layout-c4', 'Define bed dimensions - width, length, path width - consistent with growing system'),
      ck('mgd-s4-crop-rotation-bed-layout-c5', 'Define block layout relative to access and irrigation zones'),
      ck('mgd-s4-crop-rotation-bed-layout-c6', 'Map rotation blocks on zone framework'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s4-irrigation-strategy-c1', 'Define irrigation system type - drip, overhead, or combination by crop category'),
      ck('mgd-s4-irrigation-strategy-c2', 'Define irrigation zone layout - zones sized to pump capacity and pressure'),
      ck('mgd-s4-irrigation-strategy-c3', 'Define automation level - manual, timer, soil moisture sensor'),
      ck('mgd-s4-irrigation-strategy-c4', 'Define fertigation approach if applicable'),
      ck('mgd-s4-irrigation-strategy-c5', 'Confirm system type is consistent with growing philosophy and certification requirements'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s4-fertility-strategy-c1', 'Define primary fertility inputs - compost, green manures, cover crops, amendments'),
      ck('mgd-s4-fertility-strategy-c2', 'Define compost production approach - scale, feedstocks, turning frequency'),
      ck('mgd-s4-fertility-strategy-c3', 'Define green manure and cover crop species for rotation integration'),
      ck('mgd-s4-fertility-strategy-c4', 'Define external input substitution plan - what gets replaced over time'),
      ck('mgd-s4-fertility-strategy-c5', 'Define fertility monitoring indicators and testing schedule'),
      ck('mgd-s4-fertility-strategy-c6', 'Confirm all inputs are consistent with certification requirements if applicable'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s4-ipm-strategy-c1', 'Define IPM hierarchy - cultural, biological, physical, chemical as last resort'),
      ck('mgd-s4-ipm-strategy-c2', 'Define acceptable interventions consistent with certification status'),
      ck('mgd-s4-ipm-strategy-c3', 'Define beneficial insect habitat strategy - hedgerows, insectary plantings'),
      ck('mgd-s4-ipm-strategy-c4', 'Define weed management approach - cultivation timing, mulching, flame weeding'),
      ck('mgd-s4-ipm-strategy-c5', 'Define disease management approach - variety selection, air circulation, sanitation'),
      ck('mgd-s4-ipm-strategy-c6', 'Confirm all interventions are consistent with growing philosophy'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s4-post-harvest-handling-c1', 'Define wash and pack process per crop category - wash water quality, grading, pack standard'),
      ck('mgd-s4-post-harvest-handling-c2', 'Define cooling requirements per crop category - cold chain from harvest to delivery'),
      ck('mgd-s4-post-harvest-handling-c3', 'Define storage capacity requirements at peak harvest periods'),
      ck('mgd-s4-post-harvest-handling-c4', 'Define food safety requirements - HACCP, hand hygiene, contamination prevention'),
      ck('mgd-s4-post-harvest-handling-c5', 'Confirm post-harvest standard meets all market channel requirements'),
      ck('mgd-s4-post-harvest-handling-c6', 'Define maximum time from harvest to delivery per channel'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s5-bed-growing-infrastructure-c1', 'Design permanent bed layout - dimensions, orientation, path widths'),
      ck('mgd-s5-bed-growing-infrastructure-c2', 'Design tunnel or polytunnel placement - orientation, anchoring, ventilation'),
      ck('mgd-s5-bed-growing-infrastructure-c3', 'Design shade cloth and row cover infrastructure'),
      ck('mgd-s5-bed-growing-infrastructure-c4', 'Design bed edging and path surfacing'),
      ck('mgd-s5-bed-growing-infrastructure-c5', 'Design windbreak integration where required'),
      ck('mgd-s5-bed-growing-infrastructure-c6', 'Confirm layout is consistent with rotation block plan'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s5-irrigation-system-c1', 'Design mainline from source to all irrigation zones'),
      ck('mgd-s5-irrigation-system-c2', 'Design zone headers and sublines per bed block'),
      ck('mgd-s5-irrigation-system-c3', 'Design drip tape or sprinkler layout per crop category'),
      ck('mgd-s5-irrigation-system-c4', 'Design zone valve placement and control system'),
      ck('mgd-s5-irrigation-system-c5', 'Specify pipe sizes, fittings, and pressure regulation'),
      ck('mgd-s5-irrigation-system-c6', 'Confirm system delivers adequate pressure and volume at furthest point'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s5-wash-pack-cold-storage-c1', 'Design wash area - wash tanks, water supply, drainage, surface materials'),
      ck('mgd-s5-wash-pack-cold-storage-c2', 'Design packing bench and grading area - workflow from wash to pack'),
      ck('mgd-s5-wash-pack-cold-storage-c3', 'Design cold storage - capacity, temperature range, humidity control'),
      ck('mgd-s5-wash-pack-cold-storage-c4', 'Design packaging storage area'),
      ck('mgd-s5-wash-pack-cold-storage-c5', 'Specify all surfaces to food safety standard - cleanable, non-absorbent'),
      ck('mgd-s5-wash-pack-cold-storage-c6', 'Confirm design meets food safety and certification requirements'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s5-fertility-composting-infrastructure-c1', 'Design compost bay system - number, size, turning method'),
      ck('mgd-s5-fertility-composting-infrastructure-c2', 'Design worm farm if applicable - capacity, feedstock, leachate collection'),
      ck('mgd-s5-fertility-composting-infrastructure-c3', 'Design compost application equipment - spreader, wheelbarrow routes'),
      ck('mgd-s5-fertility-composting-infrastructure-c4', 'Design cover crop seed storage and seeder access'),
      ck('mgd-s5-fertility-composting-infrastructure-c5', 'Specify materials for all compost infrastructure'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s5-propagation-nursery-c1', 'Design propagation area - bench space, light, ventilation, temperature control'),
      ck('mgd-s5-propagation-nursery-c2', 'Design heat mat and germination chamber layout'),
      ck('mgd-s5-propagation-nursery-c3', 'Design irrigation for propagation area - misting, overhead'),
      ck('mgd-s5-propagation-nursery-c4', 'Design hardening off area - transition from propagation to outdoor conditions'),
      ck('mgd-s5-propagation-nursery-c5', 'Specify seed storage conditions'),
      ck('mgd-s5-propagation-nursery-c6', 'Confirm propagation capacity matches transplanting schedule at peak demand'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s6-crop-yield-monitoring-c1', 'Design harvest weight recording per bed and per crop'),
      ck('mgd-s6-crop-yield-monitoring-c2', 'Design yield-per-bed comparison system across seasons'),
      ck('mgd-s6-crop-yield-monitoring-c3', 'Define variety performance tracking - yield, quality, disease resistance'),
      ck('mgd-s6-crop-yield-monitoring-c4', 'Design crop failure recording - cause, severity, management response'),
      ck('mgd-s6-crop-yield-monitoring-c5', "Define season-end review process - what data drives next season's plan"),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s6-sales-revenue-tracking-c1', 'Design revenue tracking per crop and per channel'),
      ck('mgd-s6-sales-revenue-tracking-c2', 'Design cost-of-production tracking per bed or per crop'),
      ck('mgd-s6-sales-revenue-tracking-c3', 'Define gross margin calculation per crop category'),
      ck('mgd-s6-sales-revenue-tracking-c4', 'Define channel profitability comparison'),
      ck('mgd-s6-sales-revenue-tracking-c5', 'Define financial review frequency - monthly minimum during season'),
    ],
    decisionGroups: [
  ...
  obj({
    ...
    checklist: [
      ck('mgd-s6-adaptive-management-c1', 'Define annual season review process - yield data, revenue data, soil health reviewed together'),
      ck('mgd-s6-adaptive-management-c2', 'Define decision triggers - what performance outcome requires a plan change'),
      ck('mgd-s6-adaptive-management-c3', 'Define variety replacement protocol - what yield or quality threshold retires a variety'),
      ck('mgd-s6-adaptive-management-c4', 'Define rotation adjustment process - what disease or weed pressure triggers a rotation change'),
      ck('mgd-s6-adaptive-management-c5', 'Document all changes with season, trigger, and outcome'),
    ],
    decisionGroups: [
