...
### R2 — Gap C (s1 intent empties): wire form-arm tools — **DONE (2026-06-03)**
On implementation this proved larger than data-in-shared. The existing form-arm tools (`purpose-statement` etc.) are hardwired to the universal `s1-vision` checklist-item ids (`formId: 's1-vision-c1'…`) and universal prompts ("State the primary purpose of **this land project**"). Reusing them on, say, `hms-s1-household-needs` would open a modal whose `formId`/prompt does not match that objective's checklist. Closing R2 properly therefore required **new per-type form-catalog entries** (new prompts + new `formId`s) — authored catalogue content. The operator explicitly delegated this authoring (2026-06-03) under a "legendary biodynamics / permaculture / SaaS expert" brief.

**Convention adopted (per-checklist-item, full integration):** catalogue id == `formId` == **real checklist-item id** (e.g. `hms-s1-household-needs-c1`). The form pipeline is fully generic — `ActTierShell.handleFormSave(formId, text)` persists via `saveVisionForm` AND calls `setItemComplete(projectId, objectiveId, formId)`; because the formId IS the checklist-item id, saving a form **ticks that checklist box and advances the objective progress bar**. `VisionFormModal` renders generically from the entry's `prompt`. No modal/store edits were needed — adding a catalog `{kind:'form', formId, prompt}` entry + listing its id in the override map is sufficient.

**Scope delivered:** 222 per-item form tools across **36 s1 intent objectives** (32 primary + 4 additive secondary) spanning all 12 permaculture project types. Each objective's override flipped from intentional `[]` to its per-item form-id array. Prompts are the **verbatim** checklist-item text from each type catalogue (ASCII-normalized; the one apostrophe — `ag-s1-experience-vision-c4` "farm's" — uses a double-quoted string). Verbatim-encoding flags preserved: `con-s1-tenure-covenant` keeps "carbon credits"/"carbon agreement" in its prompts (Amanah scope flags live on the objective content, not the form tool).

Commits: `426952b1` (regen_farm + market_garden + orchard), `090a778a` (livestock + conservation + off_grid + agritourism + ecovillage + education + wellness), plus homestead (`ec71a9ad`) and silvopasture (`7e077900`) earlier in the session. Audit Gap C fell 130 → 98 (all intentional, 0 default-driven) as the 32 primary intent objectives moved out of the empty set; the 4 additive secondaries wire correctly but sit outside the primary-objective Gap C universe.

Note: the universal `s1-vision` (with its 7 form arms) is **already resolved into every project** alongside the per-type s1 objectives — so R2 adds type-specific intent capture on top of the project-level vision capture, achieving universal s1-vision parity per objective.
...
**R2 — DONE (2026-06-03)** (see §4 R2). All 36 s1 intent objectives across the 12 types now carry per-checklist-item form-arm capture tools (222 tools); each saves to its own checklist item and advances the objective progress bar. A dedicated ratchet (`every s1 intent objective resolves to per-item form-arm capture tools (R2)`, commit `b52a5cab`) locks it. Final audit: Gap A 0 / Gap B 0 / Gap C 98 (all intentional, 0 default-driven).
...
### Ecovillage — DONE (R1/R3, 2026-06-03)

**R1 (ecovillage).** All **31** `ev-*` objectives now carry explicit `OBJECTIVE_ACT_TOOLS_OVERRIDE` entries. Ecovillage is **primary-only** (`canBeSecondary: false`) — no standalone secondary layer and no patches — so this is a primary-only wiring (and primary-only ratchet). **13 map to grounded tools**, concentrated in the **S2/S3 site & systems surveys** (carrying-capacity->zone/buffer-ring/note, tenure-boundary->path/gates/fencing/note, landscape-vectors->neighbour-pin/catchment/runoff-path/hazard-zone/note, water-yield->watercourse/spring/catchment/storage/wells, waste-cycling->soil/compost/zone/watercourse, energy-potential->sun-sector/wind-sector/watercourse/vegetation/power, infra-condition->buildings/barns/roads/path/power/water-lines) and the **S5 design block** (cluster-layout->dwellings/zone/buffer-ring/path/fire-sector, communal-systems->buildings/barns, sanitation-waste->tanks/water-lines/compost/swale/buildings, energy-system->sun-sector/power/buildings/tanks, food-zones->beds/crops/orchards/water-lines/buildings/zone), plus the **S4 housing-cluster zoning framework**->zone/buffer-ring. **18 set to intentional gap-noted `[]`** — the whole S1 governance band (legal/tenure/governance, provision-balance, conflict-framework), the S2 social-fabric survey, the S4 settlement / infra / food strategies and **both financial objectives** (EV-S4.8, EV-S7.5), the S6 monitoring band (social / maintenance / coordination / external-relations protocols), and the whole S7 phasing band (settlement-plan, launch-sequence, onboarding, adaptive-management, exit-succession). Before this the ecovillage objectives fell through the coarse stratum default and surfaced the access-utilities set instead of the source / structure / climate-sector / zoning families.

**Amanah.** EV-S4.8 (financial contribution & shared economics model) and EV-S7.5 (communal financial plan & capital contribution schedule) are **communal member cost-sharing among co-owners** — member buy-in, ongoing levies, capital reserves — **NOT advance sale of future yield**, and were encoded verbatim in the catalogue per the operator's informed **2026-05-29 "encode verbatim, no gating" authorisation**. The Act layer maps both to an intentional `[]` (they are financial/governance decisions with no spatial or field-log act), so **no act surface engages a contribution instrument**. No new fiqh is re-encoded at the Act layer. Clean.

**R3 (ratchet).** `actToolCoverage.test.ts` gains an "every ecovillage objective has an explicit override entry" assertion (primary-only, like homestead / regen-farm / market-garden / conservation / off-grid / agritourism; intentional `[]` still satisfies it). 14/14.

**Audit re-run after ecovillage R1/R3:** 316 objectives — Gap A **80 → 49** (ecovillage's 31 now covered), Gap B 0, Gap C 97 → 112 (105 intentional / 7 default-driven; +18 intentional = the ecovillage governance/financial/protocol objectives, −3 default-driven now explicitly wired).

**Tests (bounded, `--pool=forks`, per-workspace):** ✅ `actToolCoverage.test.ts` 14/14 · ✅ `objectiveObserveDomains.test.ts` 8/8 · ✅ `resolveProjectObjectives.test.ts` **25/25**. Shared `tsc --noEmit` EXIT 0.

Committed `71c4671f` (code, clean 3-file explicit-pathspec commit — no index pollution) + this docs commit. **Note:** the code commit's message body miscounts the split as "19 grounded / 12 []"; the **correct split is 13 grounded / 18 intentional `[]`** (confirmed by the audit's +18 intentional delta). The code, tests and audit are correct; only the commit-message prose is wrong. Fixing it needs `--amend` (forbidden on this rebased branch), so it is left for the operator; these docs carry the authoritative counts.

**Remaining Gap A: 49**, across the 2 still-unwired primary types (education, wellness) plus the nursery/residential secondary-only objectives that do not reduce Gap A. R2 (form-arm tools) stays deferred.

### Education — DONE (R1/R3, 2026-06-03)

**R1 (education).** All **22** `edu-*` objectives now carry explicit `OBJECTIVE_ACT_TOOLS_OVERRIDE` entries. Education is **primary-only** (no standalone secondary layer and no patches) — so this is a primary-only wiring (and primary-only ratchet). **11 map to grounded tools**, concentrated in the **S2/S3 site & demo surveys** (teaching-infrastructure->buildings/zone/path/note, learning-potential->soil/watercourse/vegetation/crops/note, landscape-vectors->neighbour-pin/catchment/hazard-zone/note, learner-access-safety->path/hazard-zone/parking/fencing/buildings, demo-baseline->soil/crops/vegetation/water-lines) and the **S4/S5 teaching-zone & demo-plot design block** (teaching-zone-allocation->buildings/zone/path/beds, safety-risk-framework->hazard-zone/fencing/path, teaching-spaces->buildings/zone, demo-plots-signage->beds/crops/path/note, learner-amenity->buildings/water-lines/zone, food-kitchen->buildings/barns). **11 set to intentional gap-noted `[]`** — the whole S1 decision band (mission-audience, curriculum-programs, and the regulatory-framework hard gate), the S4 program-delivery and food-hospitality strategies, the S6 evaluation / external-relations-compliance / adaptive-management monitors, and the whole S7 band (program-launch hard gate, instructor-onboarding, financial-viability). Before this the education objectives fell through the coarse stratum default and surfaced the access-utilities set instead of the structure / zoning / survey / safety families.

**Amanah.** EDU-S7.6 (financial viability) is ordinary **fee-for-service / break-even budgeting** (course fees, grants) — no riba, no gharar, no advance sale of undelivered service. It maps to an intentional `[]` as a financial decision, so **no act surface engages a money instrument**. The S1 regulatory-framework, S4 safety-risk-framework and S7 program-launch hard gates are honoured at the catalogue layer (scopeNotes); the Act layer maps the two pure-decision gates (regulatory, program-launch) to `[]` and gives safety-risk-framework its spatial half (hazard-zone/fencing/path). No new fiqh is re-encoded at the Act layer. Clean.

**R3 (ratchet).** `actToolCoverage.test.ts` gains an "every education objective has an explicit override entry" assertion (primary-only, like homestead / regen-farm / market-garden / conservation / off-grid / agritourism / ecovillage; intentional `[]` still satisfies it). 15/15.

**Audit re-run after education R1/R3:** 316 objectives — Gap A **49 → 27** (education's 22 now covered), Gap B 0, Gap C 112 → 120 (116 intentional / 4 default-driven; +11 intentional = the education decision/financial/protocol objectives, −3 default-driven now explicitly wired).

**Tests (bounded, `--pool=forks`, per-workspace):** ✅ `actToolCoverage.test.ts` 15/15 · ✅ `objectiveObserveDomains.test.ts` 8/8 · ✅ `resolveProjectObjectives.test.ts` **25/25**. Shared `tsc --noEmit` EXIT 0.

Committed `ac98a686` (code, clean 3-file explicit-pathspec commit — no index pollution; commit-message body states the correct 11 grounded / 11 `[]` split) + this docs commit.

**Remaining Gap A: 27**, across the 1 still-unwired primary type (wellness) plus the nursery/residential secondary-only objectives that do not reduce Gap A. R2 (form-arm tools) stays deferred.

### Wellness — DONE (R1/R3, 2026-06-03) — Gap A CLOSED

**R1 (wellness).** All **32** wellness objectives now carry explicit `OBJECTIVE_ACT_TOOLS_OVERRIDE` entries: **27 `well-*` primary + 5 `well-sec-*` secondary** (wellness ships a standalone **additive secondary overlay layer**, no patches), so this is a primary+secondary wiring (and a primary+secondary union ratchet, like silvopasture / orchard). **13 map to grounded tools** (all in the primary layer), concentrated in the **S2/S3 surveys** (sensory-environment->zone/roads/neighbour-pin/note, retreat-infrastructure->buildings/dwellings/note, landscape-context->neighbour-pin/catchment/hazard-zone/high-point/note, privacy-gradient->buffer-ring/vegetation/neighbour-pin/watercourse/note, acoustic-conditions->zone/roads/neighbour-pin/note, water-features->spring/watercourse/water/note, healing-garden-ecology->vegetation/soil/sun-sector/note) and the **S5 design block** (treatment-spaces->buildings/zone, healing-garden-design->beds/vegetation/watercourse/path/zone, guest-accommodation->dwellings/zone, privacy-screening->buffer-ring/vegetation/fencing, dining-nourishment->buildings/barns), plus the **S4 privacy-zone-hierarchy** zoning framework->zone/buffer-ring. **19 set to intentional gap-noted `[]`** — the whole S1 decision band (healing-philosophy, guest-intake, the regulatory hard gate, privacy-policy), the S4 sensory-design-standards / therapeutic-program / healing-garden-strategy / safeguarding decisions, the S6 monitoring band (outcome / sensory / external-relations), the whole S7 band (program-launch hard gate, practitioner-onboarding, adaptive-management), and **every one of the 5 `well-sec-*` overlay objectives** (philosophy / regulatory / sensory-standards / program / safeguarding — all decisions the host primary's spatial survey & design work already serves). Before this the wellness objectives fell through the coarse stratum default and surfaced the access-utilities set instead of the structure / zoning / survey / screening families.

**Amanah.** Wellness is therapeutic land stewardship — guest healing, sensory design, safeguarding, practitioner standards. There is **no sales channel, advance purchase, or financing instrument**: WELL-S7.6 adaptive-management reviews "financial data" but defines no money instrument, and the catalogue carries no fee/booking objective. Nothing engages riba or gharar — clean throughout. No fiqh is re-encoded at the Act layer.

**R3 (ratchet).** `actToolCoverage.test.ts` gains an "every wellness objective has an explicit override entry" assertion over the **`WELLNESS_PRIMARY_OBJECTIVES` + `WELLNESS_SECONDARY_OBJECTIVES` union** (intentional `[]` still satisfies it). 16/16.

**Audit re-run after wellness R1/R3:** 316 objectives — Gap A **27 → 0** (every objective across all 14 types now explicitly wired), Gap B 0, Gap C 120 → 130 (**130 intentional / 0 default-driven** — no objective relies on the stratum default any longer). **This closes Gap A.**

**Tests (bounded, `--pool=forks`, per-workspace):** ✅ `actToolCoverage.test.ts` 16/16 · ✅ `objectiveObserveDomains.test.ts` 8/8 · ✅ `resolveProjectObjectives.test.ts` **25/25**. Shared `tsc --noEmit` EXIT 0.

Committed `1c737085` (code, clean 3-file explicit-pathspec commit — no index pollution; commit-message body states the correct 13 grounded / 19 `[]` split) + this docs commit.

**Remaining Gap A: 0.** Every objective in every encoded catalogue (universal + all 11 primary types + every secondary layer) now carries an explicit Act-stage tool mapping; the coarse `STRATUM_ACT_TOOLS_DEFAULT` is no longer reached by any catalogue objective (it remains as a safety net for any future un-wired objective, which the ratchet would also catch). **R2 (form-arm tools for the s1 vision objectives) stays deferred** — it needs operator-designed per-type form content, which must not be fabricated.
