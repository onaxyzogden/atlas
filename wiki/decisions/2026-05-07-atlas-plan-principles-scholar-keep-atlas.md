...
- ✅ #3 — Principle × feature-type coverage matrix landed as `plan-principle-coverage-matrix` sub-tab (`apps/web/src/v3/plan/cards/principle-verification/PrincipleCoverageMatrixCard.tsx`). Now a 12 × 9 grid (rows = Holmgren's 12, columns = zone / path / structure / transect / guild / earthwork / **crop / fertility / ecology**). The 2026-05-07 expansion added the last three columns: P3 *Obtain a Yield* now accepts `cropStore.cropAreas` evidence directly, P6 *Produce No Waste* accepts `closedLoopStore.fertilityInfra` evidence, and P10 *Use & Value Diversity* accepts `ecologyStore.ecology` observations — closing the gap where earlier the checklist forced these to be evidenced indirectly via zones or guilds. `HolmgrenChecklistCard.featureOptions` extended in lockstep so the picker offers the new types. "Well-integrated" threshold raised from ≥4 to ≥5 to scale with the wider column set. Surfaces *uncovered principles* (zero linked features), *underweight* (one type), *well-integrated* (≥5 types). No new store; pivots existing `principleCheckStore.byProject[id]` linked-feature data.
