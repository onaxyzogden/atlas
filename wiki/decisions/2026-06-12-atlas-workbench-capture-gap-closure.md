...
## Follow-on 2026-06-12 — `silv-`/`nur-sec-`/`hms-` tiers closed; committed priority list COMPLETE (commit `59869ce8`)

The final three priority tiers — **`silv-` 21 + `nur-sec-` 5 + `hms-` 32 = 58 no-path items across 11 objectives** — closed in one slice, the same way. Per-objective verification (tsx one-off against `allCatalogueObjectives()`) confirmed every item is a **decision, readiness confirmation (`Confirm… before livestock arrive`), protocol/calendar design, or estimate** — the scattered "map arm" suggestions were again vocabulary, not mechanism ("Map revenue timeline" = temporal; "Map household food needs against the site" = analytical). The orch- principle held without exception: the catalogue read is authoritative over the report heuristic.

**One flagged judgment call:** `nur-sec-s1-water-survey` (NRS-S1.2) is the membership set's first **measurement/survey-flavored** objective ("Test water quality", "Measure static and dynamic pressure") — all prior members were decision objectives. The generic per-item textarea is still an honest capture for recorded test results (narrative + value); a bespoke log/test capture stays deferred pending operator mockups. Operator approved closure via membership with no holdout.

**What shipped.** The 11 ids (`silv-s4-animal-health`, `silv-s6-animal-health-monitoring`, `silv-s7-financial-viability`, `silv-s7-livestock-establishment`, `nur-sec-s1-water-survey`, `hms-s4-energy-shelter-resilience`, `hms-s4-food-production-strategy`, `hms-s6-self-sufficiency-feedback`, `hms-s7-adaptive-management`, `hms-s7-budget-input-reduction`, `hms-s7-provision-phasing`) appended to `TIER_ZERO_OBJECTIVE_IDS` (47 → 58) + `EXPECTED_IDS`. All verified plain `ck()`, non-empty `decisionGroups`, no descriptor entry; 10 map to `[]` in `objectiveActTools.ts` and `nur-sec-s1-water-survey` is absent entirely (same classifier effect). **No render-code change.** New combined render-test block: 11-id generic mount, textarea `onRecord` (`silv-s7-livestock-establishment`), divider count (`hms-s4-food-production-strategy`, 3 groups). `silv-s7-financial-viability-c4` already had a better-ranked path and is unaffected (membership never downgrades).

**Numbers (2026-06-12 silv/nur/hms baseline):** `no-path` **440 → 382** (`silv-` **21 → 0**, `nur-sec-` **5 → 0**, `hms-` **32 → 0**); evidence-backed **474 → 532**. Web 98/98 (ActTierZeroWorkbench 86; drift guard at 58; ratchet 5/5), shared classifier 9/9. Typecheck: the only errors are in untracked operator-WIP test files (`syncServiceWorkItemsFallback.test.ts`, `WorkConflictSection.test.tsx`) — slice files clean.

**The committed priority list (universal → ev- → orch- → silv- → nur-sec- → hms-) is now COMPLETE.** Remaining ~382 no-path items sit in un-prioritized tiers (`ag-` 76, `ofg-` 66, `well-` 55, `edu-` 48, `con-` 44, `lvs-` 40, `mgd-` 16, `rf-` 10, `res-` 6, `lvs-sec-` 6) — next-tier selection there is a fresh operator decision, not covered by this ADR. Note ahead for whoever picks `ag-`: `ag-s4-revenue-model` c7–c11 is a membership/season-pass instrument with explicit bayʿ mā laysa ʿindak guardrails in the catalogue labels — Scholar Council-gated, do not close it via a generic textarea without an explicit Amanah review.

**Amanah.** Financial-flavored items (`silv-s7-financial-viability` c1/c2/c5/c6, `hms-s6-self-sufficiency-feedback-c2`, `hms-s7-budget-input-reduction` c4/c5) route through the generic textarea (narrative estimates); the bespoke Amanah-clean worksheet stays deferred under the same channel constraints. `silv-s7-financial-viability-c3` ("first sales") concerns surplus/possessed-stock sale — clean per [[fiqh-surplus-sale-clean]]. Catalogue text left verbatim.

---

## Follow-on 2026-06-12 — un-prioritized tail closed in one slice; audit no-path story COMPLETE (commit `8bb95559`)

The **entire remaining tail — 382 no-path items across 68 objectives in 10 prefixes** (`ag-` 12 objectives, `ofg-` 12, `well-`/`well-sec-` 13, `edu-` 8, `con-` 8, `lvs-` 8, `lvs-sec-` 1, `mgd-` 3, `rf-` 2, `res-` 1) — closed via workbench membership in a single slice. Two explicit **operator rulings this session** set the scope: (1) entire tail in one slice rather than tier-by-tier; (2) **include both fiqh-flagged objectives with catalogue guardrails verbatim** — that Q&A constitutes the explicit Amanah review the silv/nur/hms follow-on required before touching `ag-s4-revenue-model`.

**The determination.** Programmatic verification (tsx one-off against `allCatalogueObjectives()`): all 68 exist, **all plain `ck()` with zero special-kind items** (the investigation-stage claim that `lvs-s4-stocking-rate`/`lvs-s7-break-even` carry `ckF` formula items was wrong — no formula items exist in the tail), all carry non-empty `decisionGroups` (2–4), none appear in `workbenchAffordances.ts`. The orch-/silv- principle held a fifth time: "map arm" report vocabulary, decision-capture mechanism.

**The fiqh-flagged members (operator-ruled INCLUDE).** `ag-s4-revenue-model` c7–c11 *are* the guardrails: c7 defaults to no membership/season-pass tier, c8 constrains any tier to a membership-benefit structure (not nights-purchase — avoids bayʿ mā laysa ʿindak / gharar), c9 delivered-not-prepaid per the MGD-S1.4/S1.6 CSA guardrail, c10 carrying-capacity bound, c11 **routes any membership/season-pass instrument to Scholar Council review before adoption**. The generic textarea records the operator's decisions *against* these guardrails — it does not create, price, or sell an instrument. The c11 Scholar Council label is **pinned by render test** (verbatim catalogue text in the mounted workbench). Same logic for `lvs-s7-marketing` (c3 herd-share/CSA flag lives in scopeNotes). Catalogue text byte-untouched.

**What shipped.** 68 ids appended to `TIER_ZERO_OBJECTIVE_IDS` (**58 → 126**) + `EXPECTED_IDS`, under ten tier comment blocks. **No render-code change.** New tail render-test block: `it.each` generic mount over all 68, textarea `onRecord` (`res-s1-household-needs`), divider assertion (`ag-s4-revenue-model`) + the c11 Amanah pin. **New rendering fact pinned:** `DecisionList` emits a divider at each group-label *transition* in checklist order — `ag-s4-revenue-model`'s 4 groups are non-contiguous (6 transitions), so dividers = transitions, not `decisionGroups.length`; all prior divider tests used contiguous-group objectives where the two coincide.

**Numbers (2026-06-12 tail baseline):** `no-path` **382 → 0** (all 10 tail prefixes → 0); `workbench-capture` evidence-backed total **532 → 914**; objective-level-only stable at 1115; the same 4 known unmatched form arms remain. **The audit's gap-closure arc is COMPLETE — every in-app checklist item now has a completion path.** Web 168/168 (drift guard at 126; ratchet 5/5), shared classifier 9/9; typecheck red only on pre-existing untracked operator-WIP test files.

**Precedents extended:** survey-flavored members (`res-s1-household-needs`, `ag-s2-seasonal-patterns`) covered by the `nur-sec-s1-water-survey` precedent. Remaining work on this front is **bespoke captures only** (financial worksheets, protocol/calendar captures, water-test logs) — pending operator mockups, never invented.

**Amanah.** Classification/routing only — no instrument created, priced, or sold. `con-s7-funding-resourcing` channels (grants, biodiversity/carbon credits, trusts, partnerships) are clean. `mgd-s6-sales-revenue-tracking`/`mgd-s7-financial-viability`/`lvs-s7-break-even` record completed/possessed-stock activity — clean per [[fiqh-surplus-sale-clean]]. Any bespoke financial worksheet remains bound to donation / restricted donation / qard ḥasan / in-kind / sponsorship channels only, no CSRA/salam ([[fiqh-csra-erased-2026-05-04]], [[feedback-csa-in-catalogues]]).
