...
| ~~Bespoke per-kind inline popovers~~ | **Closed by Slice 8-E** — `buildHabitatFeatureEditSchema` in `apps/web/src/v3/plan/layers/inlineEditSchemas.ts` emits per-kind fields (`mountingHeightM` for owl-box / nest-box, `heightM` for raptor-perch, `approxHeightM` + `cavityCount` for snag) plus a conditional `hostTreeFeatureId` select for mount-on-tree kinds (`owl-box` / `raptor-perch` / `nest-box` / `snag`). `PlanSelectionFloater` renders the count chip as a button that opens the schema-driven inline popover via `useInlineFormStore.open(...)`. Empty-state surfaces a readonly select with helper copy ("Place an oak / pine / apple / shrub point first.") so stewards discover the dependency. Stewardship sovereignty preserved — the user names the host; no auto-inference |
