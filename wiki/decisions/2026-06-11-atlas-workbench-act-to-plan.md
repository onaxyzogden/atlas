...
- **Locked-objective deep-link parity LANDED (was residual #3, done 2026-06-11, `0e7d2b16`):** Plan's two routes gained `beforeLoad` guards mirroring Act's (added 2026-06-07) — `v3PlanStratumRoute` (`computeAllStratumStates`) and `v3PlanStratumObjectiveRoute` (`ctx.statuses[objectiveId] === 'locked'`) both redirect locked deep-links to the bare `/plan` landing via the shared `buildActLockContext` helper. A typed/bookmarked URL to a locked Tier-0 objective no longer mounts the workbench on a gated objective.
- **Item-level defer relabel LANDED (was residual #4, done 2026-06-11, `6efaba7b`):** the workbench's item-level deferred-state label *"Deferred — needs observation"* collided with Plan's objective-level "Deferred" status (left-rail card + "Restore") now on the same screen; relabeled `ACT_COPY.workingPanel.deferDeferred` → *"On hold — needs observation"*. Item-level defer and objective-level deferral now read distinctly.
- **Plan survey map takeover LANDED (was residual #1, done 2026-06-11, `8c6144d3`):** supersedes the original "read-only summary, no Plan map takeover" default. Arming the `s2-ecology` vegetation or `s2-terrain` slope survey in Plan now mirrors Act's full takeover — center yields to the editable `VisionLayoutCanvas` (which mounts the four reused Act survey hosts inside its private `DiagnoseMap` render-prop), right rail swaps to `VegetationSurveyPanel`/`SlopeSurveyPanel`, workbench suppressed via `&& !surveyActive && !slopeActive` on the swap flag. All four migration residuals now closed — see the second follow-on note in [[log/2026-06-11-atlas-workbench-act-to-plan]].
