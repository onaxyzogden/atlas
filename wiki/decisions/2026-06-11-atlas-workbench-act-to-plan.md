...
- **Stale copy deferred (residual #2):** `DecisionChecklist`'s "decisions are worked through in Act" banner + "Open in Act →" CTA (`open-in-act-trigger` testid) are now stale and need a follow-on copy slice.
