...
## Act CONSUMES the Threshold-3 handoff -- three Act-side surfaces (2026-06-20)

> **First deliberate Act/Plan divergence.** Every "Byte-identical under T*" section above documents Plan-only chrome. This section is the intended exception: T3 ([[decisions/2026-06-19-atlas-threshold3-act-mandate]]) assembled the handoff and armed the lock but **Act consumed none of it** -- an executing steward never saw the briefing, the live monitoring/milestones Plan authored as display-only, or the governance-approved amendments. These three surfaces close that gap and ARE the intended Act work, so the byte-identical invariant does NOT govern them. The covenant that still binds is the inverse: nothing here may gate or freeze the Act loop (display / record / append-only). ADR [[decisions/2026-06-20-atlas-act-consume-t3-handoff]]; Log [[log/2026-06-20-atlas-act-consume-t3-handoff]]. Four commits `fb05612e`/`77a8b946`/`b475db09`/`5308376c` on `main`, **not pushed**.

- **`ActMandateBriefingCard.tsx` (project-global, `77a8b946`)** -- reuses the PURE `assembleActMandate` to render the T3 handoff READ-ONLY in the Act Dashboard: Planning Direction (T1) + Coherence Record (T2) + grouped per-stratum handoffs + key documents + advisory readiness. Sources `useProjectObjectives` + `planObjectiveStatuses` (already in shell scope), `useRealityCheckStore` / `useCoherenceCheckStore` (empty fallbacks), `findPlanStratum().title` for `stratumTitleFor`, and `useActMandateStore` byProject as the render gate (only when `mandatedAt` set). Prepended in BOTH `ActOpsDashboard` branches; green `ACT_MANDATE_PALETTE` reused; no writes. `data-testid="act-mandate-briefing"`.
- **`ActObjectiveMonitoringPanel.tsx` (per-objective, `b475db09`)** -- reads `objective.monitoringProtocol?.{indicators,triggers,feeds}` (mirrors Plan `MonitoringStreamPanel`), shows the latest live reading per indicator by matching the newest `ObserveDataPoint` from `getByObjective(projectId, objective.id)` on `measurementValue.label === indicator.metric`, and adds a covenant-guarded "record reading" control writing an `ObserveDataPoint` exactly as `ActTierExecutionPanel.handleRecord` builds it (`domainId: monitoringProtocol.feeds` -- a `UniversalDomain`, no cast; `sourceObjectiveId: objective.id`; `sourceType:'manual_observation'`; `statusOutput:'clear'`; `capturedBy:'act-tier'`). The reading `note` is the ONE new free-text Act surface -> dual-boundary `detectCsaLikeText` + `CSA_ADVISORY_COPY` (UI advisory disabling submit + hard write-time reject before `recordDataPoint`). Self-gates when no `monitoringProtocol`. `data-testid="act-execution-monitoring"`.
- **`ActObjectiveLaunchProgress.tsx` + `launchMilestoneStore.ts` (per-objective, `5308376c`)** -- reads `objective.progressTracking?.milestones[]` (`{metric,cadence}`, min 2; mirrors Plan `LaunchProgressPanel`) and renders a per-milestone reached toggle (CheckCircle2/Circle, `aria-pressed`, struck-off metric + "Reached {day} . {who}" meta). The NEW `launchMilestoneStore` (key `ogden-launch-milestone-progress`, v1, idb, `byProject[projectId][objectiveId][milestoneKey] -> {reachedAt,reachedBy}`; `milestoneKey` IS the authored `metric` string, no slug) has `markReached` (idempotent), `clearReached` (remove-only), `milestonesFor`/`useObjectiveMilestones`, frozen `EMPTY_OBJECTIVE_MILESTONES`; registered `blob('ogden-launch-milestone-progress', useLaunchMilestoneStore, 'byProject', 1, byKey('byProject', null, {}))` in `syncManifest` so the build-time coverage guard passes. Record-only -> NO free text -> no covenant surface. Self-gates when no `progressTracking`. `data-testid="act-execution-progress"`.
- **`ActObjectiveAmendments.tsx` (per-objective, `fb05612e`)** -- reads `approvedAmendmentsForObjective(concerns, objective.id)` off the T3 `planConcernsStore`, mirroring Plan `ConcernAmendments` (CheckCircle2, `amendmentText`, "In response to:" observation, "Recorded {day} by {reviewedBy} -- permanent"); renders the approved amendment ALONGSIDE the unchanged catalogue objective (append-only / never-overwrite); self-gates on empty; no store writes -> no covenant surface. `data-testid="act-execution-objective-amendments"`.

All three mount in their hosts (`ActMandateBriefingCard` in `ActTierShell.tsx` `ActOpsDashboard`; the other three in `ActTierExecutionPanel.tsx`), each with its OWN `.module.css`, each self-gating to `null` on its optional substrate. The only cross-package imports under `v3/act` are the PURE, React-free `actMandateModel.js` / `coherenceCheckModel.js` (type-only `@ogden/shared`) -- no `v3/plan/**` component is imported (grep-proven); the Plan renderers were visual templates only.

Verified: web tsc foreign baseline 0-new; bounded `--pool=forks` 44/44 across the four feature suites + `launchMilestoneStore`; `syncManifest` coverage guard green WITH the new store; the monitoring suite pins both covenant boundaries; banned-term grep = exactly ONE sanctioned adversarial fixture (`ActObjectiveMonitoringPanel.test.tsx`); no visual-pass (backend offline). Amanah: the one free-text `note` is dual-boundary guarded with the reused re-export (no new regex); record/append/display-only is structural; no CSA/CSRA/salam ([[fiqh-csra-erased-2026-05-04]], [[feedback-csa-in-catalogues]]).

> **Reconciliation:** Stage 4 first landed as `47f4bacf` on `demo` (the working tree was foreign-switched to `demo` before the session resumed); per operator choice it was cherry-picked to `main` as `5308376c` via a throwaway worktree, the `demo` tree untouched. This entity update + ADR + log were authored the same way (throwaway `main` worktree) because HEAD is still on `demo`.
