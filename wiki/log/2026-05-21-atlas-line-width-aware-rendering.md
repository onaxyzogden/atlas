...
## Followup landed (fourth slice)
- **V1 BE-store facade widthM — VERIFIED LANDED + TEST-LOCKED.** The earlier "reverted by linter" note was stale: `apps/web/src/store/builtEnvironmentStore.ts` in fact carries the full passthrough in `HEAD` — all four V1 line interfaces declare `widthM?` (PowerLine/BuriedUtility/Fence/ExistingDriveway), the projection helpers read `e.existing?.widthM`, and every `add*`/`update*` threads it into the V2 `existing` block via `updateMetadata`. Confirmed via `git show HEAD:…` (the foreign WIP staged on that file is unrelated). The genuine gap was test coverage: `__tests__/builtEnvironmentAdapters.test.ts` round-tripped all four kinds but never set/asserted `widthM`, leaving it exposed to silent revert. Added two regression tests inside the `useBuiltEnvironmentStore facade` describe block — a four-kind add round-trip (asserts both the V2 `existing.widthM` write and the V1 projection read-back) and an `updatePowerLine` patch test. Verified: web tsc clean; `builtEnvironmentAdapters.test.ts` 15/15.
  - **Non-gaps for the record:** (1) V1 create-mode forms in `annotationFieldSchemas.ts` deliberately omit a Width field — BE edit-mode is intercepted to the V2 inline-edit path per the 2026-05-10 decision, so the V2 builders are the single source of the override UI. (2) `migrateObserveLine()` (builtEnvironmentStoreV2.ts) does not extract `widthM` — a theoretical cold-start nicety only (legacy line localStorage predates `widthM`); deferred.

## Remaining open followups
- _None for the width-aware line feature._ All Observe + Plan write paths persist `widthM`, the V1 facade passthrough is verified + test-locked, and the inline-edit override UI covers every drawable line kind that has one.
