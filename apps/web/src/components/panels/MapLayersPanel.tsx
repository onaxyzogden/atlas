...
export default function MapLayersPanel({ project, map: mapInstance, marker, onCenterProperty, boundaryColor, onBoundaryColorChange }: MapLayersPanelProps) {
  ...
  return (
    <div className={p.container}>
      <h2 className={p.title}>Map Layers</h2>

      {/* ── Base Map Style ──────────────────────────────────────────── */}
      <h3 className={p.sectionLabel}>Base Map Style</h3>
      <div className={`${p.section} ${p.mb24}`}>
        {([
          ...
          const active = style === ms.id;
          return (
            <button
              key={ms.id}
              onClick={() => setStyle(ms.id)}
              className={`${s.baseMapBtn} ${active ? s.baseMapBtnActive : ''}`}
            >
              <BaseMapIcon type={ms.id} active={active} />
              <div style={{ flex: 1 }}>
                <div className={`${s.baseMapBtnLabel} ${active ? s.baseMapBtnLabelActive : ''}`}>{ms.label}</div>
      ...
      {/* ── Overlay Layers ──────────────────────────────────────────── */}
      <h3 className={p.sectionLabel}>Overlay Layers</h3>
      <div className={`${p.section} ${p.mb24}`}>
        {([
      ...
      {/* ── Environmental Data ──────────────────────────────────────── */}
      <h3 className={p.sectionLabel}>Environmental Data</h3>
      <div>
        {envData.map((row) => (
          <div key={row.label} className={s.envRow}>
            <span className={s.envLabel}>{row.label}</span>
            <div className={p.row}>
              <span className={p.label}>{row.source}</span>
              <span className={`${p.badge} ${row.status === 'Active' ? p.badgeActive : p.badgePending}`}>
                {row.status}
              </span>
            </div>
          </div>
...
function BoundaryColorsSection({ project, map, boundaryColor, onBoundaryColorChange }: {
  ...
  const updateZone = useZoneStore((zs) => zs.updateZone);

  const allPaddocks = useLivestockStore((ls) => ls.paddocks);
  const paddocks = useMemo(() => allPaddocks.filter((pd) => pd.projectId === project.id), [allPaddocks, project.id]);
  const updatePaddock = useLivestockStore((ls) => ls.updatePaddock);

  const allCrops = useCropStore((cs) => cs.cropAreas);
  const crops = useMemo(() => allCrops.filter((c) => c.projectId === project.id), [allCrops, project.id]);
  const updateCrop = useCropStore((cs) => cs.updateCropArea);
  ...
  const colorRow = (label: string, color: string, onChange: (c: string) => void) => (
    <div className={s.colorRowWrap}>
      <span className={p.colorSwatch} style={{ background: color }} />
      <span className={s.colorRowLabel}>{label}</span>
      <input
        type="color"
        value={color}
        onChange={(e) => onChange(e.target.value)}
        className={p.colorInput}
      />
    </div>
  );
  ...
  return (
    <div className={p.mb24}>
      ...
      <div className={`${p.section} ${p.sectionGapSm}`}>
        ...
        {paddocks.map((pd) => colorRow(
          pd.name,
          pd.color ?? '#7A6B3A',
          (c) => { updatePaddock(pd.id, { color: c }); updateMapColor('paddock', pd.id, c); },
        ))}
