    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", minWidth: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ padding: "8px 0" }}>
          {["objectives", "recipe", "checklist"].map(s => (
            <button key={s} onClick={() => setActiveSection(s)} style={{
              ...
              fontSize: 12, fontWeight: s === activeSection ? 600 : 400,
        ...
        <div style={{ margin: "0 10px", padding: "10px 12px", background: C.bg3, borderRadius: 8, border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 10, color: C.textTertiary, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>Pile Vitals</div>
          {[
            ...
            <div key={v.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, fontFamily: F.mono, color: v.good === false ? C.red : v.good ? C.green : C.textPrimary }}>
                {v.value}
              </span>
            </div>
          ))}
        ...
        <div style={{ padding: "12px 16px", marginTop: "auto", borderTop: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 10, color: C.textTertiary, marginBottom: 6 }}>Plan progress</div>
          <div style={{ height: 3, background: C.bg4, borderRadius: 2 }}>
            <div style={{ height: "100%", width: "80%", background: C.amber, borderRadius: 2 }} />
          </div>
          <div style={{ fontSize: 10, color: C.textSecondary, marginTop: 5, fontFamily: F.mono }}>4 / 5 objectives</div>
        </div>
      </div>
      ...
      <div style={{ width: 310, flexShrink: 0, background: C.bg, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column" }}>
        {activeSection === "objectives" && (
          <>
            <div style={{ padding: "14px 14px 8px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.textPrimary }}>Planning Objectives</div>
                ...
                return (
                  <div key={obj.id} onClick={() => setSelectedObj(obj)} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: C.textPrimary, fontFamily: F.serif, lineHeight: 1.35, flex: 1 }}>{obj.title}</span>
                      <span style={{
                        fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 8,
                        background: sc + "22", color: sc, flexShrink: 0, textTransform: "uppercase", letterSpacing: "0.06em",
                      }}>{obj.status}</span>
                    </div>
                    <div style={{ fontSize: 9, color: C.textTertiary, marginTop: 4, fontFamily: F.mono }}>Tier {obj.tier}</div>
                  </div>
                );
              })}
            </div>
        ...
        {activeSection === "recipe" && (
          <>
            <div style={{ padding: "14px 14px 8px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.textPrimary }}>Feedstock Recipe</div>
              <div style={{ fontSize: 10, color: C.textTertiary, marginTop: 2 }}>7 layers · target C:N 30:1</div>
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 8px" }}>
              {/* C:N visual bar */}
              <div style={{ margin: "4px 0 12px", padding: "10px 12px", background: C.bg3, borderRadius: 8, border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 10, color: C.textTertiary, marginBottom: 6 }}>Browns vs Greens ratio</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                  <span style={{ fontSize: 9, color: C.amber, fontFamily: F.mono }}>Browns (C) ~80%</span>
                  <span style={{ fontSize: 9, color: C.green, fontFamily: F.mono }}>Greens (N) ~20%</span>
                </div>
              </div>
              ...
              {pile.layers.map((layer, i) => (
                <div key={layer.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 9, color: C.textTertiary, fontFamily: F.mono, marginTop: 1 }}>
                      {layer.depth} · C:N ~{layer.cnApprox}:1 · {layer.type === "brown" ? "Brown" : "Green"}
                    </div>
                  </div>
                  <div style={{ fontSize: 10, color: layer.type === "brown" ? C.amber : C.green, fontWeight: 700 }}>
                    L{i + 1}
                  </div>
                </div>
              ))}
        ...
        {activeSection === "objectives" && selectedObj ? (
          <div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 16 }}>
              <span style={{ fontSize: 10, color: C.textTertiary, fontFamily: F.mono }}>Tier {selectedObj.tier}</span>
            ...
            <h2 style={{ fontSize: 20, fontFamily: F.serif, color: C.textPrimary, fontWeight: 500, marginBottom: 20, lineHeight: 1.3 }}>{selectedObj.title}</h2>

            {/* Completion gate */}
            <div style={{ padding: "14px 16px", background: C.bg3, borderRadius: 10, border: `1px solid ${C.border}`, marginBottom: 16 }}>
              <div style={{ fontSize: 10, color: C.textTertiary, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Completion Gate</div>
              <p style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.6 }}>{selectedObj.gate}</p>
            </div>
            ...
            {selectedObj.status === "complete" && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: C.greenDim, borderRadius: 10, border: `1px solid ${C.green}44` }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.green }}>Approved for Act</div>
                  <div style={{ fontSize: 10, color: C.textSecondary, marginTop: 1 }}>Plan output verified. Act tasks have been generated from this objective.</div>
                </div>
              </div>
            )}
            ...
            {selectedObj.status === "available" && (
              <div style={{ padding: "12px 16px", background: C.amberDim, borderRadius: 10, border: `1px solid ${C.amber}44`, marginTop: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.amber, marginBottom: 4 }}>Ready to begin</div>
                <div style={{ fontSize: 11, color: C.textSecondary }}>All prerequisite objectives are complete. This objective can now be actioned.</div>
              </div>
            )}

            <div style={{ padding: "14px 16px", background: C.bg3, borderRadius: 10, border: `1px solid ${C.border}`, marginTop: 16 }}>
              <div style={{ fontSize: 10, color: C.textTertiary, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Observe Feeds</div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: C.textTertiary, fontSize: 12 }}>
            Select an objective
          </div>
        ) : (
          <div style={{ maxWidth: 480 }}>
            <h2 style={{ fontSize: 18, fontFamily: F.serif, color: C.textPrimary, fontWeight: 500, marginBottom: 12 }}>
