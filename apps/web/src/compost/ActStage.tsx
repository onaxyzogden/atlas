    ...
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minWidth: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 16px 10px', borderBottom: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.green, marginBottom: 2 }}>Act</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: C.textPrimary, fontFamily: F.serif }}>Batch 1 Execution</div>
          <div style={{ fontSize: 12, color: C.textSecondary, marginTop: 2 }}>Day {lastReading?.day ?? 0} · {lastReading?.date ?? ''}</div>
        </div>
        ...
        <div style={{ margin: '0 10px 8px', padding: '10px 12px', background: C.bg3, borderRadius: 8, border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 12, color: C.textTertiary, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Current State</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: C.textTertiary }}>Last temp</span>
            <span style={{ fontSize: 14, fontWeight: 700, color: phase.color, fontFamily: F.mono }}>{fToCStr(lastReading?.temp ?? 68)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: C.textTertiary }}>Phase</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: phase.color }}>{phase.label}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: C.textTertiary }}>Turnings</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary, fontFamily: F.mono }}>{turnings}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 12, color: C.textTertiary }}>Moisture est.</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.textPrimary, fontFamily: F.mono }}>{lastReading?.moisture ?? 0}%</span>
          </div>
        </div>
            ...
            {ACT_TASKS.map((task) => {
              ...
              return (
                <div key={task.id} onClick={() => setSelectedTask(task)} style={{
                  ...
                  <div style={{ fontSize: 13, fontWeight: isSel ? 600 : 400, color: isSel ? C.textPrimary : C.textSecondary, lineHeight: 1.35 }}>{task.title}</div>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 3 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: taskStatusColor, textTransform: 'uppercase' }}>{task.status.replace('_', ' ')}</span>
                    <span style={{ fontSize: 12, color: C.textTertiary, fontFamily: F.mono }}>{task.proofItems}/{task.proofRequired} proof</span>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0' }}>
            {[...logEntries].reverse().slice(0, 12).map((r) => (
              <div key={r.id} style={{
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: getPhaseMeta(r.temp).color, fontFamily: F.mono }}>{fToCStr(r.temp)}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ width: 310, flexShrink: 0, background: C.bg, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column' }}>
          ...
          <>
            <div style={{ padding: '14px 14px 10px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 }}>
                <span style={{
                  fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 8, textTransform: 'uppercase',
                ...
                <span style={{ fontSize: 12, color: C.textTertiary, fontFamily: F.mono }}>Phase: {selectedTask.phase}</span>
              </div>
              <div style={{ fontSize: 16, fontWeight: 700, color: C.textPrimary, fontFamily: F.serif, lineHeight: 1.3 }}>{selectedTask.title}</div>
              <div style={{ fontSize: 12, color: C.textSecondary, marginTop: 4, lineHeight: 1.5 }}>{selectedTask.desc}</div>
            </div>

            {/* Proof bar */}
            <div style={{ padding: '10px 14px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                <span style={{ fontSize: 12, color: C.textTertiary }}>Proof items</span>
                <span style={{ fontSize: 12, fontFamily: F.mono, color: selectedTask.proofItems >= selectedTask.proofRequired ? C.green : C.amber }}>
        ...
        {logView === 'log' && (
          <>
            <div style={{ padding: '14px 14px 10px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.textPrimary }}>Log a Reading</div>
              <div style={{ fontSize: 12, color: C.textTertiary, marginTop: 2 }}>Day {(lastReading?.day ?? 0) + 1} · Enter today's probe reading</div>
            </div>
            <div style={{ padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontSize: 12, color: C.textTertiary, display: 'block', marginBottom: 5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  Core Temperature (°C)
                </label>
                <input
                  type="number"
                  ...
                  style={{
                    ...
                    borderRadius: 8, color: C.textPrimary, fontSize: 17,
                    fontFamily: F.mono, outline: 'none',
                  }}
                />
                {newTemp && !isNaN(parseFloat(newTemp)) && (
                  <div style={{ marginTop: 5, fontSize: 12, color: getPhaseMeta(parseFloat(newTemp) * 9 / 5 + 32).color, fontFamily: F.mono }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: F.sans,
              ...
              <div style={{ marginTop: 4 }}>
                <div style={{ fontSize: 12, color: C.textTertiary, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Proof Schema — Daily Log</div>
                {['Temperature measurement (required)', 'Field note (required)', 'Photo of probe in situ (required)'].map((s, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 12, color: i === 0 ? C.textPrimary : C.textSecondary }}>{s}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minWidth: 0 }}>
        <div style={{ padding: '14px 20px 10px', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, background: C.bg, zIndex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.textPrimary }}>Full Temperature Log</div>
          <div style={{ fontSize: 12, color: C.textTertiary, marginTop: 1 }}>{logEntries.length} readings · {turnings} turnings · {daysAbovePasteurisation(logEntries)} days above 55°C</div>
        </div>
        <div style={{ padding: '8px 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '52px 70px 70px 60px 60px 1fr', gap: 8, padding: '4px 8px', marginBottom: 4 }}>
            {['Day', 'Date', 'Temp °C', 'Moisture', 'Turn', 'Notes'].map((h) => (
              <span key={h} style={{ fontSize: 12, color: C.textTertiary, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</span>
            ))}
          </div>
          {logEntries.map((r) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                <span style={{ fontSize: 13, color: C.textTertiary, fontFamily: F.mono }}>D{r.day}</span>
                <span style={{ fontSize: 13, color: C.textSecondary, fontFamily: F.mono }}>{r.date}</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: pm.color, fontFamily: F.mono }}>{fToCStr(r.temp)}</span>
                <span style={{ fontSize: 13, color: C.textSecondary, fontFamily: F.mono }}>{r.moisture}%</span>
                <span style={{ fontSize: 12, color: r.turned ? C.amber : C.textTertiary, fontWeight: r.turned ? 700 : 400 }}>
                  {r.turned ? '↺ Yes' : '—'}
                </span>
                <span style={{ fontSize: 12, color: C.textTertiary, lineHeight: 1.4 }}>{r.note || ''}</span>
              </div>
            );
          })}
        </div>
