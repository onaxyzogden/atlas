...
export default function RunoffPathTool({ map, projectId, getSnapTargets }: Props) {
  ...
  const place = (geom: GeoJSON.LineString) => {
    const id = createWithDefaults(FIELD_SCHEMAS.runoffPath, {
      projectId,
      geometry: geom,
    });
    if (id)
  ...
  return (
    <div className={css.popover} role="dialog" aria-label="Runoff path">
      <span className={css.title}>Runoff path</span>
      <span className={css.hint}>
        Trace observed overland flow (Freehand) or set Length × Bearing (Dimensions).
      </span>
