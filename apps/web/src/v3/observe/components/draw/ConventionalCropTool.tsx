...
export default function ConventionalCropTool({ map, projectId }: Props) {
  ...
  const place = (geom: GeoJSON.Polygon) => {
    const id = createWithDefaults(FIELD_SCHEMAS.conventionalCrop, {
      projectId,
      geometry: geom,
    });
    if (id)
      open({
        kind: 'conventionalCrop',
        geometry: geom,
        mode: 'edit',
        existingId: id,
        projectId,
  ...
  return (
    <div className={css.popover} role="dialog" aria-label="Conventional crop">
      <span className={css.title}>Conventional crop</span>
      <span className={css.hint}>
        Outline conventionally-farmed ground (Freehand) or set Width × Depth / Radius (Dimensions).
      </span>
