...
import styles from './SunPathDiagram.module.css';
...
export default function SunPathDiagram({ lat, className }: Props) {
  if (lat == null || Number.isNaN(lat)) {
    return (
      <div className={`${styles.diagram} ${styles.empty} ${className ?? ''}`}>
  ...
  function arc(altDeg: number, label: string, cls: 'summer' | 'equinox' | 'winter') {
    const r = (altDeg / 90) * rMax;
    const d = `M ${cx - rMax} ${cy} A ${rMax} ${r} 0 0 1 ${cx + rMax} ${cy}`;
    return (
      <g key={label}>
        <path d={d} className={`${styles.sunArc} ${styles[cls]}`} fill="none" />
        <text x={cx} y={cy - r - 4} textAnchor="middle" className={styles.sunLabel}>
  ...
  return (
    ...
    >
      <line x1={16} y1={cy} x2={W - 16} y2={cy} className={styles.horizonLine} />
      {arc(summer, 'Jun', 'summer')}
      {arc(equinox, 'Mar/Sep', 'equinox')}
      {arc(winter, 'Dec', 'winter')}
      <text x={20} y={cy + 14} className={styles.sunAxis}>
        E
      </text>
      <text x={W - 26} y={cy + 14} className={styles.sunAxis}>
        W
      </text>
    </svg>
  );
