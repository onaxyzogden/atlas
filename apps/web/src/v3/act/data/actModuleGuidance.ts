/**
 * actModuleGuidance — net-new Operations-Scholar guidance for the Act stage,
 * mirroring PLAN_MODULE_GUIDANCE / MODULE_GUIDANCE. Each module gets a `why`
 * (rationale), `how` (3-4 concrete steps grounded in the module's real cards),
 * and a `pitfall` (the common failure mode). The `how` steps reference the
 * actual Act card sections (see MODULE_CARDS in ../types.ts) so the compass
 * checklist maps onto work the steward can actually do.
 *
 * Net-new copy for the Act Stage Compass (2026-05-24, Goal 6, Phase 4).
 * The permaculture-principle attributions (Holmgren, Yeomans, Savory,
 * Mollison) are the author's drafting and remain flagged for Scholar
 * review before being treated as doctrinal.
 */

import type { GuidanceCardData } from '../../_shared/components/GuidanceCard.js';
import type { ActModule } from '../types.js';

export const ACT_MODULE_GUIDANCE: Record<ActModule, GuidanceCardData> = {
  tracker: {
  ...
  maintain: {
    why: 'A permaculture system is only self-regulating if its infrastructure keeps running — irrigation, fences, compost flows, and machinery degrade silently until a failure cascades. The maintenance module logs events and schedules upkeep so problems arrive as feedback, not as crises (Holmgren P4: Apply self-regulation and accept feedback).',
    how: [
    ...
    pitfall:
      'Running infrastructure to failure instead of on a schedule — reactive repair costs more, and a missed irrigation or broken waste loop can take a whole guild with it.',
  },
  livestock: {
  ...
  review: {
    why: 'A design locked at Plan time decays as climate, market, and land change. The review module keeps an ongoing SWOT and live hazard plans so the steward steers by current reality, not the year-zero diagnosis (Holmgren P4: Apply self-regulation and accept feedback).',
    how: [
    ...
    pitfall:
      'Treating SWOT and hazard plans as a one-time Observe artifact — an un-revisited risk register is a false sense of safety.',
  },
  network: {
