 ...
 * Act Mandate). These tests pin the "render the APPROVED amendment ALONGSIDE the
 * original, ONLY" contract:
 *   - an objective with no approved amendment renders nothing (self-gates);
 *   - an approved concern's amendment surfaces on its objective with the
 *     observation context and a permanent recorded date + reviewer;
 ...
 *
 * The overlay reads the real planConcernsStore, exactly as it does at runtime.
 */
...
import ConcernAmendments from '../ConcernAmendments.js';

const PID = 'project-concern-overlay-1';
...
describe('ConcernAmendments', () => {
  it('renders nothing for an objective with no approved amendment', () => {
    const { container } = render(
      <ConcernAmendments projectId={PID} objectiveId={OBJ} />,
    );
    expect(
  ...
  it('surfaces an approved amendment alongside the original, with context + provenance', () => {
    ...
    render(<ConcernAmendments projectId={PID} objectiveId={OBJ} />);

    expect(
      screen.getByTestId('objective-act-mandate-amendments'),
    ).toBeTruthy();
    expect(screen.getByTestId('concern-amendment-concern-1')).toBeTruthy();
    expect(
      screen.getByText(ACT_MANDATE_COPY.onObjective.label),
    ).toBeTruthy();
    expect(
