...
import { totalHoursPerWeek } from '../../v3/observe/modules/human-context/derivations.js';
...
export default function DiagnosisReportExport({ project }: Props) {
  ...
  const markdown = useMemo(() => {
    ...
    // Steward roster — identity from members, profile overlay from visionStore.
    const profiles = visionData?.stewardProfiles ?? {};
    const roster = members.map((m) => ({ member: m, profile: profiles[m.userId] ?? {} }));
    lines.push(`## 1. Human Context`);
    lines.push('');
    lines.push(`**Stewards:** ${roster.length}`);
    if (roster.length) {
      const totalHrs = roster.reduce((sum, r) => sum + totalHoursPerWeek(r.profile), 0);
      lines.push(`- Combined capacity: ${totalHrs} hrs/wk`);
    }
    lines.push('');
    for (const { member, profile } of roster) {
      const name = member.displayName || member.email.split('@')[0];
      lines.push(`### ${name}${profile.relationship ? ` — ${profile.relationship}` : ''} (${member.role})`);
      ...
      if (profile.maintenanceHrsInitial !== undefined)
        lines.push(`- Maintenance hrs/wk (initial): ${profile.maintenanceHrsInitial}`);
      if (profile.maintenanceHrsOngoing !== undefined)
        lines.push(`- Maintenance hrs/wk (ongoing): ${profile.maintenanceHrsOngoing}`);
      if (profile.budget) lines.push(`- Budget: ${profile.budget}`);
      if (profile.skills?.length) lines.push(`- Skills: ${profile.skills.join(', ')}`);
      if (profile.personalVision) {
        lines.push('');
        lines.push(`> ${profile.personalVision.replace(/\n/g, '\n> ')}`);
      }
      lines.push('');
    }

    // Shared vision package (project-level).
    const sv = visionData?.sharedVision;
    if (sv?.statement || sv?.coreFunctions?.length || sv?.successMetrics?.length) {
      lines.push(`### Shared Vision`);
      if (sv?.statement) {
        lines.push('');
        lines.push(`> ${sv.statement.replace(/\n/g, '\n> ')}`);
      }
      if (sv?.coreFunctions?.length)
        lines.push(`- **Core functions:** ${sv.coreFunctions.join(', ')}`);
      if (sv?.experienceGoals?.length)
        lines.push(`- **Experience goals:** ${sv.experienceGoals.join(', ')}`);
      if (sv?.successMetrics?.length)
        lines.push(`- **Success metrics:** ${sv.successMetrics.join(', ')}`);
    }
    ...
    return lines.join('\n');
  }, [project, visionData, members, siteData, soilSamples, hazards, transects, sectors, ecology, successionStage, swot]);
