...
import { AiDesignSupportResponse } from '@ogden/shared';

/**
 * Section 18 — AI-Assisted Design Support ([P3])
 *
 * Read-path returns the typed AiDesignSupportResponse envelope. The matching
...
export default async function ai_design_supportRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: AiDesignSupportResponse.parse({
          status: 'not_ready',
