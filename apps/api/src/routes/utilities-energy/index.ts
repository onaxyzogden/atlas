...
import { UtilitiesEnergyResponse } from '@ogden/shared';

/**
 * Section 13 — Utilities, Energy & Support Systems ([P2])
 *
 * Read-path returns the typed UtilitiesEnergyResponse envelope. The matching
...
export default async function utilities_energyRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: UtilitiesEnergyResponse.parse({
          status: 'not_ready',
