...
import { PublicPortalResponse } from '@ogden/shared';

/**
 * Section 27 — Public Experience & Storytelling Portal ([P4])
 *
 * Read-path returns the typed PublicPortalResponse envelope. The matching
...
export default async function public_portalRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    '/:projectId',
    { preHandler: [authenticate, fastify.requirePhase('P4'), resolveProjectRole] },
    async (req) => {
      return {
        data: PublicPortalResponse.parse({
          status: 'not_ready',
