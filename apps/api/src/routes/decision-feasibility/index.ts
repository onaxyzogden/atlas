...
import { DecisionFeasibilityResponse } from '@ogden/shared';

/**
 * Section 21 — Decision Support & Feasibility ([P2])
 *
 * Read-path returns the typed DecisionFeasibilityResponse envelope. The matching
...
export default async function decision_feasibilityRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: DecisionFeasibilityResponse.parse({
          status: 'not_ready',
