...
import { MoontranceVisionResponse } from '@ogden/shared';

/**
 * Section 14 — Moontrance Vision Layer & Concept Overlay ([P2])
 *
 * Read-path returns the typed MoontranceVisionResponse envelope. The matching
...
export default async function moontrance_visionRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: MoontranceVisionResponse.parse({
          status: 'not_ready',
