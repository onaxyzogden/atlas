...
import { SiteAssessmentResponse } from '@ogden/shared';

/**
 * Section 4 — Site Assessment & Diagnostic Atlas ([P1])
 *
 * Read-path returns the typed SiteAssessmentResponse envelope. The matching
...
export default async function site_assessmentRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: SiteAssessmentResponse.parse({
          status: 'not_ready',
