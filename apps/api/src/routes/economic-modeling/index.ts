...
import { EconomicModelingResponse } from '@ogden/shared';

/**
 * Section 22 — Economic Planning & Business Modeling ([P2])
 *
 * Read-path returns the typed EconomicModelingResponse envelope. The matching
...
export default async function economic_modelingRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: EconomicModelingResponse.parse({
          status: 'not_ready',
