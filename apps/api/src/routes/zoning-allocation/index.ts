...
import { ZoningAllocationResponse } from '@ogden/shared';

/**
 * Section 8 — Land Use Zoning & Functional Allocation ([P1])
 *
 * Read-path returns the typed ZoningAllocationResponse envelope. The matching
...
export default async function zoning_allocationRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: ZoningAllocationResponse.parse({
          status: 'not_ready',
