...
import { CropsAgroforestryResponse } from '@ogden/shared';

/**
 * Section 12 — Crop, Orchard & Agroforestry Design ([P2])
 *
 * Read-path returns the typed CropsAgroforestryResponse envelope. The matching
...
export default async function crops_agroforestryRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: CropsAgroforestryResponse.parse({
          status: 'not_ready',
