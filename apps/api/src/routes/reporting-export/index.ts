...
import { ReportingExportResponse } from '@ogden/shared';

/**
 * Section 23 — Reporting, Export & Presentation ([P2])
 *
 * Read-path returns the typed ReportingExportResponse envelope. The matching
...
export default async function reporting_exportRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: ReportingExportResponse.parse({
          status: 'not_ready',
