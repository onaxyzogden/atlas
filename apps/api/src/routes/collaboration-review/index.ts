...
import { CollaborationReviewResponse } from '@ogden/shared';

/**
 * Section 20 — Collaboration, Teamwork & Review ([P3])
 *
 * Read-path returns the typed CollaborationReviewResponse envelope. The matching
...
export default async function collaboration_reviewRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: CollaborationReviewResponse.parse({
          status: 'not_ready',
