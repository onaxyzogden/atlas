...
import { SiteDataLayersResponse } from '@ogden/shared';

/**
 * Section 3 — Site Data Layers & Environmental Inputs ([P1])
 *
 * Read-path returns the typed SiteDataLayersResponse envelope. The matching
...
export default async function site_data_layersRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { projectId: string } }>(
    ...
    async (req) => {
      return {
        data: SiteDataLayersResponse.parse({
          status: 'not_ready',
