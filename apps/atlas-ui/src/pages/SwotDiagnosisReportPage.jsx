...
const metadata = screenCatalog.find((s) => s.route === "/observe/swot/diagnosis-report");

const RATING_DOTS = (n) => "•".repeat(Math.max(0, Math.min(5, n))) + "·".repeat(5 - Math.max(0, Math.min(5, n)));
...
export function SwotDiagnosisReportPage() {
  ...
  return (
    <div className="verdean-shell">
      <VerdeanRail siteName={siteBanner?.title ?? "351 House"} siteAreaHa={meta.siteAreaHa ?? siteBanner?.areaHa} />
      <main className="diagnosis-report-page">
        <ReportTopbar siteName={siteBanner?.title ?? "351 House"} />
        <ReportStageBar />
        <section className="diagnosis-report-frame">
          <header className="diagnosis-report-hero">
            ...
            <SurfaceCard className="report-generated-card">
              ...
              <span>Report generated<small>May 3, 2026 - 10:32 AM</small></span>
            </SurfaceCard>
          </header>
...
function VerdeanRail({ siteName, siteAreaHa }) {
  const process = [
    ...
    ["2", "Record", true],
    ["3", "Analyse", true],
    ["6", "SWOT Synthesis", false],
  ];
  const design = [["7", "Concepts"], ["8", "Design plan"], ["9", "Implementation"]];
  return (
    <aside className="verdean-rail">
      <div className="verdean-logo">
        <Target aria-hidden="true" />
        <span>Verdean<br />Design</span>
      </div>
      <button className="verdean-project" type="button">
        <b>{siteName}</b>
        <span>{siteAreaHa ? `${siteAreaHa} ha` : "Atlas Sample"}</span>
      ...
      {process.map(([num, label, done]) => (
        <button className={label === "SWOT Synthesis" ? "is-active" : ""} type="button" key={label}>
          <span>{num}</span>{label}{done ? <Check aria-hidden="true" /> : <i />}
        </button>
      ))}
      <div className="verdean-subnav">
        <Link to="/observe/swot/journal">SWOT journal</Link>
        <span className="is-active">Diagnosis report</span>
      </div>
      <h2>Design</h2>
      {design.map(([num, label]) => (
        <button type="button" key={label}><span>{num}</span>{label}</button>
      ))}
...
function ReportTopbar({ siteName }) {
  return (
    <header className="report-topbar">
      ...
      <nav>
        <button type="button"><Download aria-hidden="true" /> Export report</button>
        <button type="button"><Share2 aria-hidden="true" /> Share summary</button>
        <button className="green-button" type="button"><Plus aria-hidden="true" /> Add to design plan</button>
      </nav>
    </header>
  );
}
...
function ReportStageBar() {
  return (
    <SurfaceCard className="report-stage-bar">
      {["Observe", "Record", "Analyse", "Synthesize", "Integrate"].map((item, index) => (
        <span className={index < 3 ? "is-done" : index === 3 ? "is-active" : ""} key={item}>
          <b>{index < 3 ? <Check aria-hidden="true" /> : index + 1}</b>{item}
        </span>
      ))}
    </SurfaceCard>
  );
...
function ExecutiveSummary({ meta }) {
  const facts = [
...
function ReportRadar({ values }) {
  // Diamond chart: S top, W right, T bottom, O left. Max radius = 80, centre = 90,90.
  const r = 80;
  const cx = 90, cy = 90;
...
function TopInsights() {
  return (
    <SurfaceCard className="report-card top-insights">
      ...
      <Link to="/observe/swot/journal" className="report-link">See full analysis in the SWOT journal <ArrowRight aria-hidden="true" /></Link>
    </SurfaceCard>
  );
}
  ...
  return (
    <SurfaceCard className="report-card findings-card">
      ...
      {vm.provisionalFindings.map((row) => (
        <p key={row.title} className={row.category}>
          <Leaf aria-hidden="true" />
          <span>{row.title}<small>From SWOT synthesis — category: {row.category}.</small></span>
          <b>{RATING_DOTS(row.rating)}</b>
          <b>{RATING_DOTS(Math.min(5, row.rating + 1))}</b>
          <ArrowRight aria-hidden="true" />
        </p>
      ))}
      <Link to="/observe/swot/journal" className="report-link">View all findings in SWOT journal <ArrowRight aria-hidden="true" /></Link>
    </SurfaceCard>
  );
}
  ...
  return (
    <SurfaceCard className="report-card risk-flags-report">
      ...
      {vm.riskFlags.map((row) => (
        <p key={row.title} className={`risk-${row.level}`}>
...
function EvidenceCard() {
  return (
    <SurfaceCard className="report-card evidence-card">
      ...
      {vm.evidenceFromJournal.map((row, i) => (
        <p key={row.title} className={row.category}>
          <time>{row.date}</time>
          {row.title}
          <b>{row.category[0].toUpperCase()}{i + 1}</b>
        </p>
      ))}
      <Link to="/observe/swot/journal" className="report-link">Open SWOT journal <ArrowRight aria-hidden="true" /></Link>
    </SurfaceCard>
  );
}
    ...
    <SurfaceCard className="report-card report-actions-card">
      ...
      {vm.recommendedActions.map((row) => (
        <p key={row.title}>
          ...
          <span>{row.title}</span>
          <b className={`priority-${row.priority.toLowerCase()}`}>{row.priority}</b>
          <time>{row.due}</time>
        </p>
      ))}
